...
#include "mqtt_client.h" // Add this line to include the MQTT client header
#include "esp_sleep.h"
...
void wifi_init_sta(void) {
    ...
    ip_info.ip.addr = CONFIG_IP_ADDRESS;
    ip_info.gw.addr = CONFIG_GATEWAY_IP_ADDRESS;
    ip_info.netmask.addr = CONFIG_SUBNET_MASK;
...
void mqtt_app_start_and_send(void) {
    ...
    char msg[100];
    sprintf(msg, "From boot to send time: %ld ms", esp_log_timestamp());

    esp_mqtt_client_publish(client, CONFIG_MQTT_TOPIC, msg, 0, 1, 0);
